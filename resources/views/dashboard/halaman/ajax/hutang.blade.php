<script>
    $(document).ready(function() {
            ...
            $('#rfidModal').one('shown.bs.modal', function () {
                ...
                $('#rfidInput').on('input', function() {
                    ...
                    if (rfid.length >= 2) {
                        ...
                        $.ajax({
                            url: "{{ env('APP_URL').'/ajax_tambah_hutang' }}", // Ganti dengan URL tujuan
                            type: 'POST',
                            headers: {
        ...
        $('#rupiah_bayar').on('input', function() {
            let angka = $(this).val().replace(/[^,\d]/g, ""); // Hanya angka
            let rupiah = formatRupiah(angka);
            $(this).val(rupiah);
        });

        $('#uang_bayar').on('submit', function(e) {
            e.preventDefault();

            // Ambil nilai tanpa karakter selain angka
            let angkaBersih = $('#rupiah_bayar').val().replace(/[^,\d]/g, "");
            if (angkaBersih === ''|| angkaBersih < 1) {
                // alert('Angka kosong');
                $('#rfidErrorModalLabel').text('Gagal');
                $('#rfidErrorMessage').text('Jumlah uang kosong');
            ...
            let jumlah = angkaBersih;
            $('#rfidModal_bayar').modal('show');

            // Set fokus pada input RFID setelah modal benar-benar ditampilkan
            $('#rfidModal_bayar').on('shown.bs.modal', function () {
                    $('#rfidInput_bayar').focus();
            });

            // Tangkap input RFID hanya sekali saat modal pertama kali dibuka
            $('#rfidModal_bayar').one('shown.bs.modal', function () {
                $('#rfidInput_bayar').val(''); // Kosongkan input setiap kali modal ditampilkan
                ...
                $('#rfidInput_bayar').on('input', function() {
                    ...
                    if (rfid.length >= 2) {
                        ...
                        // Kirim data jumlah dan RFID melalui AJAX
                        $.ajax({
                            url: "{{ env('APP_URL').'/ajax_bayar_hutang' }}", // Ganti dengan URL tujuan
                            type: 'POST',
                            headers: {
                                'X-Requested-With': 'XMLHttpRequest',
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')  // Token CSRF
            ...
            // Kosongkan nilai input RFID ketika modal ditutup agar tidak terjadi duplikasi input
            $('#rfidModal_bayar').on('hidden.bs.modal', function () {
                $('#rfidInput_bayar').val('');
                $('#rfidInput_bayar').off('input'); // Lepaskan event handler untuk menghindari duplikasi
            });
        });
...
<div class="modal fade" id="rfidModal_bayar" tabindex="-1" aria-labelledby="rfidModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="rfidModalLabel">Tap RFID</h5>
            ...
            <div class="modal-body">
                Silakan tap RFID USB Anda...
                <input type="text" id="rfidInput_bayar" class="form-control mt-3" placeholder="Scan RFID" autofocus>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
            </div>
